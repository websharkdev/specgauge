    ...
    return <div className="flex gap-4">
        ...
        <div className="flex flex-col gap-2.5 flex-1 max-w-xl">
            ...
            <motion.h6
                ...
                className="text-sm leading-snug text-[#111111] opacity-50">{subtitle}</motion.h6>
        </div>
    </div>
}
            ...
            <div className="lg:col-span-5 col-span-full flex flex-col relative overflow-hidden h-max justify-between lg:pl-20 pl-3.5 gap-6 xl:gap-10 2xl:gap-20">
                <div className="flex flex-col gap-4 max-w-lg">
                    <motion.h5
                        ...
                        className="text-transparent bg-clip-text bg-gradient-to-r from-[#0B9C36] to-[#175F49] text-sm">Real-Time Advantage</motion.h5>
                    <motion.h2
                        initial={{ opacity: 0, }}
                        animate={isInView ? { opacity: 1, } : {}}
                        transition={{
                        ...
                        className="md:text-4xl text-3xl font-medium leading-snug text-[#111111] max-w-[353px]">Why oil companies choose SpecGauge</motion.h2>
                </div>
                <div className="flex flex-col gap-5 xl:gap-8">
                    <WhyUsItem index={1} isInView={isInView} icon={<HandCoins size={24} color="#111111" />} iconBG="linear-gradient(205.6deg, #EBF3F9 11.6%, #F9F0F0 83.8%)" title={"Save costs\n\on every delivery"} subtitle="Cut wasted trips, reduce fuel consumption, and maximize the efficiency of every delivery run — saving your depot money with every mile driven." />
                    <WhyUsItem index={2} isInView={isInView} icon={<Building2 size={24} color="#111111" />} iconBG="linear-gradient(211.73deg, #F3F5FC 9.55%, #F2F0F4 80.9%)" title={"Increase reliability\n\at critical facilities"} subtitle="Ensure that hospitals, care homes, and essential services never face fuel shortages, giving your customers complete peace of mind and building long-term trust" />
                    <WhyUsItem index={3} isInView={isInView} icon={<ClockPlus size={24} color="#111111" />} iconBG="linear-gradient(203.58deg, #EEF7F1 15.19%, #EAF2F7 91.35%)" title={"Boost efficiency\n\with real-time data"} subtitle="Plan optimized delivery schedules based on live tank information, combining multiple stops into one run and transforming daily logistics into smooth operations" />
